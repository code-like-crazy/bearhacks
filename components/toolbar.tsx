...
import { Camera, Eraser, Pencil, StickyNote, Stamp, Text } from "lucide-react"
...
export default function ToolBar({ activeTool, onToolChange, currentColor, onColorChange }: ToolBarProps) {
  const tools = [
    { id: "sticky", icon: <StickyNote className="h-6 w-6" />, label: "Sticky Note", showColorPicker: true },
    { id: "camera", icon: <Camera className="h-6 w-6" />, label: "Upload Image", showColorPicker: false },
    { id: "pencil", icon: <Pencil className="h-6 w-6" />, label: "Draw", showColorPicker: true },
    { id: "text", icon: <Text className="h-6 w-6" />, label: "Add Text", showColorPicker: false },
    { id: "eraser", icon: <Eraser className="h-6 w-6" />, label: "Erase", showColorPicker: false },
    { id: "stamp", icon: <Stamp className="h-6 w-6" />, label: "Stamps", showColorPicker: false },
    {
      id: "shape",
      icon: (
  ...
  return (
    <TooltipProvider>
      <div className="flex justify-center p-4 border-t bg-background">
        <div className="flex items-center gap-2 p-2 bg-muted/30 rounded-full shadow-sm">
          {tools.map((tool) => (
            <Tooltip key={tool.id}>
              <Popover>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {tool.shapeOptions.map((shape) => (
                        <Button
                          ...
                          size="icon"
                          className="h-8 w-8"
                          onClick={() => onToolChange(`shape-${shape.id}` as ToolType)}
                        >
                          {shape.icon}
                        </Button>
                      ))}
