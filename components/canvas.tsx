...
import React, { useRef, useState, useCallback } from "react"; // Import React
...
import { Trash2, Camera, Upload } from "lucide-react"; // Added Camera and Upload icons
...
export default function Canvas({ scale, position, activeTool, isDragging, currentColor }: CanvasProps) {
  ...
  const [imageDialogPosition, setImageDialogPosition] = useState<Point | null>(null); // Position for the image choice popover
  ...
  const addStampElement = useCallback((x: number, y: number) => {
    const stamps = ["❤️", "👍", "⭐", "✅", "🔥"];
  ...
  const placePendingImage = useCallback((point: Point) => {
  ...
  }, [pendingImageSrc, nextZIndex, setElements, setNextZIndex, setSelectedElement, setPendingImageSrc, setIsPlacingImage]);
  ...
  }, [activeTool, getCoordinatesFromEvent, startDrawing, startShape, isDragging, setShowImageChoiceDialog, setPointerDownPos, setImageDialogPosition]);
  ...
  }, [activeTool, getCoordinatesFromEvent, draw, updateShapePreview, setCursorPosition]); // Added setCursorPosition dependency

  const handlePointerUp = useCallback((e: React.PointerEvent) => { // Add event parameter 'e'
    ...
    if (pointerDownPos && endPoint) {
        ...
        if (Math.sqrt(dx * dx + dy * dy) < 5) { // Click threshold
            isClick = true;
        }
    ...
    if (isShapeDragging) setIsShapeDragging(false);
      ...
      activeTool, endDrawing, endShape, isPlacingImage, placePendingImage,
      addStickyNote, addTextElement, addStampElement, pointerDownPos,
      getCoordinatesFromEvent, isDrawing, isShapeDragging, setSelectedElement, // Added setSelectedElement
      setPointerDownPos, setIsDrawing, setIsShapeDragging // Added state setters
  ]);
  ...
  return (
    <div
      ...
      style={{
        ...
        transformOrigin: "0 0",
         cursor: isPlacingImage ? 'copy' : (activeTool === "pencil" ? "crosshair" : activeTool === "eraser" ? "not-allowed" : "default"),
       }}
    ...
    >
      ...
      <Popover open={showImageChoiceDialog} onOpenChange={setShowImageChoiceDialog}>
        {/* No PopoverTrigger needed, opened programmatically */}
        <PopoverContent
          className="w-auto p-2 border shadow-lg rounded-lg bg-white"
          style={imageDialogPosition ? {
            position: 'absolute',
        ...
        >
          <div className="flex flex-col space-y-2">
             ...
             <label
                ...
                className="flex items-center justify-start w-full px-3 py-1.5 text-sm rounded hover:bg-gray-100 transition-colors cursor-pointer"
              ...
              <button
                className="flex items-center justify-start w-full px-3 py-1.5 text-sm rounded hover:bg-gray-100 transition-colors"
                onClick={() => {
                  // TODO: Implement "Take Photo" functionality
                  console.log("Take Photo clicked - functionality not implemented");
       ...
       {isPlacingImage && cursorPosition && (
         <div
           className="absolute bg-black text-white text-xs px-2 py-1 rounded pointer-events-none whitespace-nowrap"
           style={{
