...
import { uploadFileToUploadThing } from "@/services/actions/upload-file";
...
const LiveCanvas = forwardRef<LiveCanvasRef, LiveCanvasProps>(
  (
    ...
    ref: ForwardedRef<LiveCanvasRef>,
  ) => {
    // Correctly typed ref
    // Refs
    ...
    const triggerGenerationApiCall = async () => {
      ...
      try {
        ...
        onGeminiResponse?.(
          `Error generating trip plan: ${error instanceof Error ? error.message : "Network error or invalid response"}`,
        );
      }
    };
    ...
    const syncObjectToStorage = useMutation(
      ({ storage }, object) => {
        if (!object) return;
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        // Upload the file using our server action
        console.log("Uploading file to UploadThing...");

        const result = await uploadFileToUploadThing(file);

        console.log("uploadFileToUploadThing result:", result);
        ...
        // Create an image element with the uploaded URL
        ...
        imgElement.onload = () => {
          ...
          const left = canvas.width! / 2 - (imgWidth * scaleFactor) / 2;
          const top = canvas.height! / 2 - (imgHeight * scaleFactor) / 2;

          // Create a fabric image using the HTML element
          ...
          (fabricImage as any).originalFile = {
            name: file.name,
            type: file.type,
            size: file.size,
          ...
          (fabricImage as any).src = result.url;
          (fabricImage as any).alt = file.name;
          ...
          // Sync to storage
          syncObjectToStorage(fabricImage);
          ...
          <button
            onClick={() => {
              console.log("Current Canvas Objects:", {
                ...
                objects: Array.from(canvasObjects.entries()).map(
                  ([id, obj]) => ({
                    id,
                    type: obj.type,
                    ...obj,
